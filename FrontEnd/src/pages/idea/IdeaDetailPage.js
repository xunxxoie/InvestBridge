...
const IdeaDetailPage = () => {
  ...
  if (isLoading) return (<Flex justify="center" align="center" height="100vh"><CircularProgress isIndeterminate color="blue.300" /></Flex>);
          ...
          <Container maxW="container.lg" mt={8}>
            <Box maxWidth={1000} margin="auto" mb={4}>
              <Box borderRadius="lg" overflow="hidden" boxShadow="md" bg="white">
                <Box bgImage={`linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url(${image})`} bgSize="cover" bgPosition="center" height={230} display="flex" flexDirection="column" justifyContent="center" alignItems="center" p={4} color="white" textShadow="1px 1px 3px rgba(0,0,0,0.5)">
                  <Text fontSize="6xl" fontWeight="bold" mb={3}>{idea.title}</Text>
                  <Text fontSize="l" fontWeight="bold">{idea.userId}</Text>
                </Box>
                <Box p={4}>
                  <Flex justifyContent="space-between" alignItems="center" mb={3}>
                    <Box><Button leftIcon={<FaUsers />} variant="outline" colorScheme="blue">{idea.userId}</Button></Box>
                  ...
                  <Flex flexWrap="wrap" gap={2}>
                    {(idea.categories || []).map((tag, index) => (
                      <Button key={index} size="sm" variant="outline">#{tag}</Button>
                    ))}
                  </Flex>
                </Box>
              </Box>
              <Flex mt={4} alignItems="center" justifyContent="flex-end" marginTop={10}>
                <Text fontSize="xl" fontWeight="bold" color="gray.600" mr={2}>이 아이디어가 맘에 든다면?</Text>
                <Button leftIcon={<FaRocket />} colorScheme="purple" size="lg" borderRadius="full" px={6} fontWeight="bold" _hover={{ transform: 'translateY(-2px)', boxShadow: 'lg' }} transition="all 0.3s">
                  아이디어 실현하기 🚀
                </Button>
              </Flex>
            </Box>
