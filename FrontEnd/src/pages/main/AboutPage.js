...
const AboutPage = () => {
  ...
  useEffect(() => {
    const fetchVersions = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/patchnote/patchnotes-versions`);
        if (!response.ok) {
  ...
  const fetchPatchNote = async (version) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/about/${version}`);
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
